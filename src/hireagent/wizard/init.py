"""HireAgent first-time setup wizard.

Interactive flow that creates ~/.hireagent/ with:
  - resume.txt (and optionally resume.pdf)
  - profile.json
  - searches.yaml
  - .env (local Ollama + optional CapSolver)
"""
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell HireAgent about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_searches() -> None:
    ...
    # Build YAML content
    lines = [
        "# HireAgent search configuration",
...
def _setup_ai_features() -> None:
    """Ask about AI scoring/tailoring — local Ollama configuration."""
    console.print(Panel(
        "[bold]Step 4: AI Features (local Ollama)[/bold]\n"
        "A local Ollama model powers scoring, tailoring, and cover letters.\n"
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]hireagent init[/bold].[/dim]")
        return

    env_lines = ["# HireAgent configuration", ""]
    provider = "ollama"
    model = Prompt.ask("Ollama model", default="llama3:8b")
    ...
    # Preserve existing CapSolver key if already configured.
    if ENV_PATH.exists():
...
def _setup_auto_apply() -> None:
    ...
    console.print(Panel(
        ...
        "HireAgent can autonomously fill and submit job applications\n"
        "using Claude Code as the browser agent."
    ))

    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes HireAgent generates.[/dim]")
        return
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# HireAgent configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]HireAgent Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]hireagent init[/bold].",
            border_style="green",
        )
    )
    ...
    from hireagent.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure local Ollama (re-run [bold]hireagent init[/bold]).[/dim]"
    elif tier == 2:
