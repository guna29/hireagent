...
parses salary ranges, and stores everything in the HireAgent database.
...
from hireagent.database import get_connection, init_db, store_jobs
...
def _load_location_config(search_cfg: dict) -> tuple[list[str], list[str]]:
    ...
    Supports both flat keys (location_accept / location_reject_non_remote)
    and the nested structure (location.accept_patterns / location.reject_patterns).
    """
    # Flat keys take precedence
    accept = search_cfg.get("location_accept", [])
    reject = search_cfg.get("location_reject_non_remote", [])

    # Fall back to nested location block used by searches.yaml
...
def _title_ok(title: str | None, exclude_titles: list[str]) -> bool:
    """Return False if the title contains any excluded keyword."""
    if not title or not exclude_titles:
        return True
    t = title.lower()
    return not any(ex.lower() in t for ex in exclude_titles)


def store_jobspy_results(conn: sqlite3.Connection, df, source_label: str,
                         exclude_titles: list[str] | None = None) -> tuple[int, int]:
    ...
    for _, row in df.iterrows():
        ...
        # Filter out wrong seniority/role titles at discovery time
...
def _run_one_search(
    ...
    conn = get_connection()
    new, existing = store_jobspy_results(conn, df, s["query"], exclude_titles=exclude_titles)
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `hireagent init` to create one.")
