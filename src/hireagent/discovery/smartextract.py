...
from hireagent.config import CONFIG_DIR
from hireagent.database import get_connection, init_db, store_jobs, get_stats
from hireagent.llm import get_enrich_client
...
def _load_location_filter(search_cfg: dict | None = None):
    ...
    # Also check nested location block (searches.yaml uses location.accept_patterns)
    if not accept and not reject:
        loc_block = search_cfg.get("location", {})
        if isinstance(loc_block, dict):
            accept = loc_block.get("accept_patterns", [])
            reject = loc_block.get("reject_patterns", [])
...
STRATEGY_REPAIR_PROMPT = """Convert this output to STRICT JSON only.

Output schema:
{{"strategy":"json_ld|api_response|css_selectors","reasoning":"<=20 words","extraction":{{...}}}}

Rules:
- Return ONLY JSON, no markdown, no prose.
- If uncertain, use:
{{"strategy":"css_selectors","reasoning":"fallback parse repair","extraction":{{}}}}
...
def _fallback_strategy_from_raw(raw: str, intel: dict) -> dict:
    """Keyword fallback when model ignores strict JSON format."""
    lowered = (raw or "").lower()
    if "json_ld" in lowered and any(
        isinstance(j, dict) and j.get("@type") == "JobPosting" for j in intel.get("json_ld", [])
    ):
        return {
            "strategy": "json_ld",
            "reasoning": "fallback keyword parse",
            "extraction": {
                "title": "title",
    ...
    if "api_response" in lowered and intel.get("api_responses"):
        first_url = intel["api_responses"][0].get("url", "")
        url_pattern = first_url.split("?")[0] if first_url else ""
        return {
    ...
    return {"strategy": "css_selectors", "reasoning": "fallback parse repair", "extraction": {}}


def _parse_strategy_plan(raw: str, intel: dict) -> tuple[dict, str]:
    """Parse strategy JSON with repair + deterministic fallback."""
    try:
        return extract_json(raw), "direct"
    except Exception as first_err:
        log.warning("Strategy parse failed, attempting repair: %s", first_err)

    # Attempt one repair pass through LLM using a strict JSON reformatter prompt.
    try:
        repair_raw, _, _ = ask_llm(STRATEGY_REPAIR_PROMPT.format(raw=raw[:4000]))
        plan = extract_json(repair_raw)
        return plan, "repaired"
    except Exception as repair_err:
        log.warning("Strategy repair failed, using fallback strategy: %s", repair_err)

    return _fallback_strategy_from_raw(raw, intel), "fallback"
...
def _run_one_site(name: str, url: str) -> dict:
    ...
    plan, parse_mode = _parse_strategy_plan(raw, intel)
    if parse_mode != "direct":
        log.warning("Strategy parse mode: %s", parse_mode)
...
def build_scrape_targets(
    ...
    us_only = country in {"us", "usa", "united states", "united states of america"}
    ...
    for site in sites:
        ...
        if us_only:
            site_hint = f"{site_name} {site_url}".lower()
            if any(token in site_hint for token in ("canada", "jobbank.gc.ca", ".ca/", ".ca?")):
                log.info("Skipping non-US source for US-only search: %s (%s)", site_name, site_url)
                continue
