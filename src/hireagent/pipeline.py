"""HireAgent Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    hireagent run                        # all stages, sequential
    hireagent run --stream               # all stages, concurrent
    hireagent run discover enrich        # specific stages
    hireagent run score tailor cover     # LLM-only stages
    hireagent run --dry-run              # preview without executing
"""
...
from hireagent.database import init_db, get_connection, get_stats
...
STAGE_META: dict[str, dict] = {
    "discover": {"desc": "Job discovery (JobSpy + Workday + ScoutBetter + WaaS + Wellfound + Otto + smart extract)"},
    "enrich":   {"desc": "Detail enrichment (full descriptions + apply URLs)"},
    "score":    {"desc": "LLM scoring (fit 1-10)"},
    "tailor":   {"desc": "Resume tailoring (LLM + validation)"},
    "cover":    {"desc": "Cover letter generation"},
    "pdf":      {"desc": "PDF conversion (tailored resumes + cover letters)"},
    "apply":    {"desc": "Auto-apply (browser agent fills ATS forms per job)"},
}
...
def _run_discover(workers: int = 1) -> dict:
    ...
    skip_workday = os.environ.get("HIREAGENT_SKIP_WORKDAY", "").strip().lower() in {"1", "true", "yes", "on"}
    if skip_workday:
        console.print("  [yellow]Workday scraper skipped (HIREAGENT_SKIP_WORKDAY=1).[/yellow]")
        stats["workday"] = "skipped"
    else:
        console.print("  [cyan]Workday corporate scraper...[/cyan]")
        try:
            from hireagent.discovery.workday import run_workday_discovery
    ...
    # ScoutBetter (only if credentials are configured)
    if os.environ.get("SCOUTBETTER_EMAIL") and os.environ.get("SCOUTBETTER_PASSWORD"):
        console.print("  [cyan]ScoutBetter discovery...[/cyan]")
        try:
            from hireagent.discovery.scoutbetter import run_discovery as sb_discover
            sb_discover()
            stats["scoutbetter"] = "ok"
        except Exception as e:
            log.error("ScoutBetter discovery failed: %s", e)
            console.print(f"  [red]ScoutBetter error:[/red] {e}")
            stats["scoutbetter"] = f"error: {e}"
    else:
        stats["scoutbetter"] = "skipped (no credentials)"

    # Work at a Startup (YC)
    skip_waas = os.environ.get("HIREAGENT_SKIP_WAAS", "").strip().lower() in {"1", "true", "yes", "on"}
    if skip_waas:
        console.print("  [yellow]Work at a Startup scraper skipped (HIREAGENT_SKIP_WAAS=1).[/yellow]")
        stats["workatastartup"] = "skipped"
    else:
        console.print("  [cyan]Work at a Startup (YC) scraper...[/cyan]")
        try:
            from hireagent.discovery.workatastartup import run_workatastartup_discovery
            result = run_workatastartup_discovery()
            stats["workatastartup"] = f"ok ({result.get('new', 0)} new)"
        except Exception as e:
            log.error("Work at a Startup scraper failed: %s", e)
            console.print(f"  [red]WaaS error:[/red] {e}")
            stats["workatastartup"] = f"error: {e}"

    # Wellfound (startup jobs)
    skip_wellfound = os.environ.get("HIREAGENT_SKIP_WELLFOUND", "").strip().lower() in {"1", "true", "yes", "on"}
    if skip_wellfound:
        console.print("  [yellow]Wellfound scraper skipped (HIREAGENT_SKIP_WELLFOUND=1).[/yellow]")
        stats["wellfound"] = "skipped"
    else:
        console.print("  [cyan]Wellfound startup jobs scraper...[/cyan]")
        try:
            from hireagent.discovery.wellfound import run_wellfound_discovery
            result = run_wellfound_discovery()
            stats["wellfound"] = f"ok ({result.get('new', 0)} new)"
        except Exception as e:
            log.error("Wellfound scraper failed: %s", e)
            console.print(f"  [red]Wellfound error:[/red] {e}")
            stats["wellfound"] = f"error: {e}"

    # Otto.careers (AI-focused jobs)
    skip_otto = os.environ.get("HIREAGENT_SKIP_OTTO", "").strip().lower() in {"1", "true", "yes", "on"}
    if skip_otto:
        console.print("  [yellow]Otto.careers scraper skipped (HIREAGENT_SKIP_OTTO=1).[/yellow]")
        stats["otto"] = "skipped"
    else:
        console.print("  [cyan]Otto.careers AI jobs scraper...[/cyan]")
        try:
            from hireagent.discovery.otto import run_otto_discovery
            result = run_otto_discovery()
            stats["otto"] = f"ok ({result.get('new', 0)} new)"
        except Exception as e:
            log.error("Otto.careers scraper failed: %s", e)
            console.print(f"  [red]Otto error:[/red] {e}")
            stats["otto"] = f"error: {e}"

    # Smart extract
    if skip_smart_extract:
        console.print("  [yellow]Smart extract skipped (HIREAGENT_SKIP_SMART_EXTRACT=1).[/yellow]")
        stats["smartextract"] = "skipped"
    else:
        console.print("  [cyan]Smart extract (AI-powered scraping)...[/cyan]")
        try:
            from hireagent.discovery.smartextract import run_smart_extract
...
def _run_score() -> dict:
    ...
    try:
        from hireagent.scoring.scorer import run_scoring
        run_scoring()
        return {"status": "ok"}
    except Exception as e:
...
def _run_tailor(min_score: int = 0, validation_mode: str = "normal") -> dict:
    """Stage: Resume tailoring — generate tailored resumes for ALL eligible jobs regardless of score."""
    try:
        from hireagent.scoring.tailor import run_tailoring
        run_tailoring(min_score=min_score, validation_mode=validation_mode, limit=500)
        return {"status": "ok"}
    except Exception as e:
...
def _run_cover(min_score: int = 0, validation_mode: str = "normal") -> dict:
    """Stage: Cover letter generation for ALL tailored jobs."""
    try:
        from hireagent.scoring.cover_letter import run_cover_letters
...
def _run_pdf() -> dict:
    ...
    try:
        from hireagent.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def _run_apply(min_score: int = 0, headless: bool = False, model: str = "haiku",
               dry_run: bool = False) -> dict:
    """Stage: Auto-apply — process each eligible job one-by-one through the browser agent."""
    try:
        from hireagent.apply.launcher import main as apply_main
        apply_main(
            limit=0,          # process all available jobs
            min_score=min_score,
            headless=headless,
            model=model,
            dry_run=dry_run,
            continuous=False, # stop when queue drains (don't poll forever)
            workers=1,
        )
        return {"status": "ok"}
    except Exception as e:
...
def _count_pending(stage: str, min_score: int = 0) -> int:
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]HireAgent Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
